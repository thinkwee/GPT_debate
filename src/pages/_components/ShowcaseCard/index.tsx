...
function ShowcaseCard({ user, isDescription, onCopy, onLove }) {
  ...
  async function handleCopyClickRed() {
    try {
      const response = await fetch(`http://localhost:666/count/red/${user.id}`, { method: 'POST' });
  ...
  async function handleCopyClickBlue() {
    try {
      const response = await fetch(`http://localhost:666/count/blue/${user.id}`, { method: 'POST' });
      const data = await response.json();
      setCopiedBlue(true);
  ...
  async function fetchCounts() {
    try {
      const responseRed = await fetch(`http://localhost:666/count/red/${user.id}`);
      const responseBlue = await fetch(`http://localhost:666/count/blue/${user.id}`);
